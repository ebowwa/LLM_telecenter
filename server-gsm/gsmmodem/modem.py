...
class GsmModem(SerialComms):
    ...
    CMGR_REGEX_PDU = None
    # Add this line to define the regex pattern for +CMGR notifications
    CMGR_REGEX_NOTIFICATION = re.compile(r'^\+CMGR:\s*(\d+),"(\+{0,1}\d+)",.*,".*",(.*)$')
    ...
    def __init__(self, port, baudrate=115200, incomingCallCallbackFunc=None, smsReceivedCallbackFunc=None, smsStatusReportCallback=None, requestDelivery=False, AT_CNMI="", *a, **kw):
    ...
    def sendUssd(self, ussdString, responseTimeout=15):
        """Starts a USSD session by dialing the specified USSD string, or \
        sends the specified string in the existing USSD session (if any)

        :param ussdString: The USSD access number to dial
        :param responseTimeout: Maximum time to wait for a response, in seconds
        ...
        """
        print(f"Sending USSD command: {ussdString}")
        ...
        try:
            ...
            print(f"Raw response from modem: {cusdResponse}")
        except Exception:
    ...
    def _handleSmsReceived(self, notificationLine): 
        ...
        if self.smsReceivedCallback is not None:
            ...
            if cmtiMatch:
                ...
                self.log.debug(f"Reading SMS with index {msgIndex} from memory {msgMemory}")
                try:
                    sms = self.readStoredSms(msgIndex, msgMemory)
                except Exception as e:
                    self.log.error(f"Failed to read SMS with index {msgIndex} from memory {msgMemory}: {e}")
                    return
                try:
                    self.smsReceivedCallback(sms)
                except Exception:
    ...
    def readStoredSms(self, index, memory=None):
        ...
        self.log.debug(f"Raw message data for index {index}: {msgData}")
